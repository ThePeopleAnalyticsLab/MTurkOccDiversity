...
unzip("BLS2015.zip")
library(readxl)
national_M2015_dl <- read_excel("oesm15nat/national_M2015_dl.xlsx")
View(national_M2015_dl)

#Edit the code below such that the newly downloaded data aligns with the code below. You'll need to include relative percentages. 

BLS <- read_excel("~/Documents/Research/Research on MTurk/MTurk's Occupational Diversity/Datasets/BLS Data from 2015.xlsx")
...
names(BLS) <- c("Var1","Var2","Var3")
BLS <- aggregate(.~Var1,BLS,sum)
###Data on the size of the military taken from the department of defense (http://us-military-branches.insidegov.com). 
BLS[13, 2] = "1350236"
names(BLS) <- c("Intermediate Aggregation Title", "Freq","RFreq")
BLS$Freq <- as.numeric(BLS$Freq)
tab <- transform(BLS, relative = prop.table(Freq))
BLS <- tab[c(1,4)]
names(BLS) <- c("Intermediate Aggregation Title","e")
```

```{r Sample Data, include=FALSE, results='asis', echo=FALSE}
##LATech data
LATech <- read_csv("/Users/ccasti02/Documents/Research/Research on MTurk/MTurk's Occupational Diversity/Datasets/Chris's Data from 2015.csv")
...
##UGA data
UGA <- read_excel("/Users/ccasti02/Documents/Research/Research on MTurk/MTurk's Occupational Diversity/Datasets/Rachel's Data from 2015.xlsx")
UGA <- UGA[c(2,21)]
